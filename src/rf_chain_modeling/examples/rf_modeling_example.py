...
from rf_chain_modeling.rf_utils.rf_modeling import Signals, calculate_spectral_rms_dbm, thermal_noise_power_dbm
...
def main() -> None:
    ...
    for fr, pow, ph in tones:
        logger.info("Adding %6.3f GHz tone at %6.3f dBm", fr / 1e9, pow)
        signal.add_tone(fr, pow, ph)
        logger.info("signal.rms_dbm() after adding %4.1f GHz tone: %6.2f dBm", fr / 1e9, signal.rms_dbm())

    # Print RMS value
    logger.info("%7.2f dBm in whole spectrum",
                calculate_spectral_rms_dbm(signal.spectrums.mean(axis=0),
                                           signal.imped_ohms))

    for fr, pow, ph in tones:
        logger.info("%7.2f dBm in spectrum at %6.3f GHz",
                    signal.rms_at_freq_dbm(fr), fr / 1e9)
